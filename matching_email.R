...
tabelle <- read.csv(file = input_incoming, header=T, na.strings = c("","NA"))
...
for(i in 1:nrow(tabelle)){
  recipients <- c(toString(tabelle[i,Spalte_EMail_Incoming]),toString(tabelle[i,Spalte_EMail_Tuebingen]))
  send.mail(from = sender,
            ...
            body = paste("Hello there!",sep="\n", "Congratulations, you two have been matched as Exchange Buddies for the Winter Semester 2019/20!",sep="\n",
                         ...
                         "First Name: ", toString(tabelle[i, Spalte_Vorname_Incoming]), sep="\n",
                         "Last Name: ", toString(tabelle[i, Spalte_Nachname_Incoming]),sep="\n",
                         "Gender: ", toString(tabelle[i, Spalte_Geschlecht_Incoming]), sep="\n",
                         "Home Country: ", toString(tabelle[i, Spalte_Land_Incoming]), sep="\n",
                         "Home University: ", toString(tabelle[i, Spalte_Uni_Incoming]), sep="\n",
                         "Study Program: ", toString(tabelle[i, Spalte_Studienfach_Incoming]),
                         zweites_Fach(tabelle[i, Spalte_Studienfach2_Incoming]),
                         "E-Mail Address: ", toString(tabelle[i,Spalte_EMail_Incoming]) ,sep="\n",
                         "Expected Arrival Date: ", toString(tabelle[i, Spalte_Datum_Incoming]), sep="\n",
                         "Participation in Start or Kompakt Kurs?: ", toString(tabelle[i, Spalte_Kurs_Incoming]),sep="\n",
                         "Preffered communication language: ", toString(tabelle[i, Spalte_comlang_Incoming]), sep="\n",
                         
                         underline("Tuebinger Buddy:"), sep="\n",
                         "First Name: ", toString(tabelle[i, Spalte_Vorname_Tuebingen]), sep="\n",
                         "Last Name: ", toString(tabelle[i, Spalte_Nachname_Tuebingen]),sep="\n",
                         "Gender: ", toString(tabelle[i, Spalte_Geschlecht_Tuebingen]), sep="\n",
                         "Study Program: ", toString(tabelle[i, Spalte_Studienfach_Tuebingen]),
                         zweites_Fach(tabelle[i, Spalte_Studienfach2_Tuebingen]),
                         "E-Mail Address: ", toString(tabelle[i,Spalte_EMail_Tuebingen]) ,sep="\n"),
            "Available in Tuebingen from: ", toString(tabelle[i, Spalte_Datum_Tuebingen]), sep="\n",
