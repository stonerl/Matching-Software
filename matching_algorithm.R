...
matching_programm <- function()
{
  ...
  if(anzahl_tuebinger < anzahl_incomings){
      for(j in 1:anzahl_tuebinger){
        if(toString(tabelle_tuebingen[j,Spalte_2_Buddys]) == "No"){#es wird ueberprueft, ob der Tuebinger nur einen Buddy betreuen moechte
          deleted_cols <- c(deleted_cols,j) #falls ja, so merken wir uns die Person
        }
      }
    if(length(deleted_cols)>0) punkte_matrix <- punkte_matrix[,-deleted_cols] #alle Tuebinger, die nur 1 Buddy betreuen moechten, werden aus Matrix geloescht
    
     for (i in 1:anzahl_incomings){
        if(!(is.na(matching$proposals[i,1]))){#es wird ueberprueft, ob der Incoming schon gematcht wurde
          deleted_rows <- c(deleted_rows, i) #falls ja, so merken wir uns die Person
        }
      }
   
    if(length(deleted_rows)>0) {punkte_matrix <- punkte_matrix[-deleted_rows,]} #alle Incomings, die schon gematcht wurden, werden aus Matrix geloescht
    
    index_matrix <- vektor_neue_indizes(deleted_cols, anzahl_tuebinger) #nun muessen die Indizes der aktualisierten Matrix angepasst werden (vgl. vektor_neue_indizes)
    ...
    for(k in 1:anzahl_incomings){
      ...
      if(is.na(buddy_matching[k,1])){ #falls der Incoming beim ersten Matching ungematched blieb
        # wird nun im zweiten Matching nachgesehen, zu wem er gematcht wurde (u.U. immer noch NA)
       
        if(!(is.na(matching_2$proposals[index,1]))){
        buddy_matching[k,1] <- reihe_index_umrechnung(matching_2$proposals[index,1], index_matrix)}
        index <- index + 1
      }
    }
  ...
  ausfuehrliche_uebersicht <- data.frame(list(tabelle_incoming[1,],tabelle_tuebingen[buddy_matching[1],]))
  ...
  auswahl_incoming = c(Spalte_Nachname_Incoming, Spalte_Alter_Incoming, Spalte_Studienfach_Incoming, Spalte_Studienfach2_Incoming, 
                       Spalte_Studienabschluss_Incoming, Spalte_Uni_Incoming, Spalte_Hobby1_Incoming, Spalte_Hobby2_Incoming, 
                       Spalte_Hobby3_Incoming, Spalte_Datum_Incoming)
  auswahl_tuebingen = c(Spalte_Nachname_Tuebingen, Spalte_Alter_Tuebingen, Spalte_Studienfach_Tuebingen, Spalte_Studienfach2_Tuebingen,
                       Spalte_Studienabschluss_Tuebingen, Spalte_Uni_Tuebingen, Spalte_Hobby1_Tuebingen, Spalte_Hobby2_Tuebingen, 
                       Spalte_Hobby3_Tuebingen, Spalte_Datum_Tuebingen)
  
  Kurzuebersicht <- data.frame(list(tabelle_incoming[1,auswahl_incoming],tabelle_tuebingen[buddy_matching[1],auswahl_tuebingen]))
  for(k in 2:nrow(buddy_matching)){
...
punkte_algorithmus <- function(incoming, tuebingen){#header wird mit uebergeben
  ...
  if(studienfach_tue == studienfach_in){ #falls die Teilnehmer dasselbe studieren
    score <- score+Studienfach_Gewichtung #vergib Punktzahl fuer dasselbe Studienfach
  } else if(studienfach_fachbereich_tue == studienfach_fachbereich_in){ #falls Teilnehmer am selben Fachbereich studieren
    score <- score+Fachbereich_Gewichtung # vergib Punktzahl fuer denselben Fachbereich
  } else if(studienfach_fakultaet_tue == studienfach_fakultaet_in){ # falls Teilnehmer an der selben Fakultaet studieren
    score <- score+Fakultaet_Gewichtung # vergib Punktzahl fuer dieselbe Fakultaet
  }else if(berg_tal(studienfach_fakultaet_tue, studienfach_fakultaet_in)){
    score <- score + BergTal_Gewichtung # vergib Punktzahl falls beide am Berg oder im Tal studieren
  }
  
  land = incoming[Spalte_Land_Incoming]
  
  if((studienfach_tue == "Japanologie // Japanese Studies") & (land == "Japan")){
    score <- score + Land_Studiengang_Gewichtung
  }
  if((studienfach_tue == "Koreanistik // Korean Studies") & (land == "South Korea")){
    score <- score + Land_Studiengang_Gewichtung
  }
  if((studienfach_tue == "Indologie // Indology") & (land == "India")){
    score <- score + Land_Studiengang_Gewichtung
  }
  if((studienfach_tue == "Sinologie // Chinese Studies") & 
     (land == "China" | land == "Taiwan, R.O.C." | land == "Hong Kong, SAR" | land == "Singapore" | land == "Macau")){
    score <- score + Land_Studiengang_Gewichtung
  }
 ...
 # if((studienfach_tue == "Anglistik // English Studies" | studienfach_tue == "Englisch // English language" | studienfach_tue == "English Linguistics") 
  ...
  if((studienfach_tue == "Skandinavistik // Scandinavian Studies") & 
     (land == "Denmark" | land == "Sweden" | land == "Norway" | land == "Finland" | land == "Iceland")){
       score <- score + Land_Studiengang_Gewichtung
     }
  
  if((studienfach_tue == "Slavistik // Slavic Studies" | studienfach_tue == "Russisch // Russian Language") & 
    (land == "Latvia" | land == "Lithuania" | land == "Estonia" | land == "Russia" | land == "Ukraine" | land == "Belarus" 
     | land == "Poland" | land == "Slovakia" | land == "Slovenia" | land == "Czech Republic" | land == "Croatia" | land == "Serbia"
     | land == "Bulgaria" | land == "Romania")){
       score <- score + Land_Studiengang_Gewichtung
  }
  
 if((studienfach_tue == "Romanistik // Romance Language and Literature" | studienfach_tue == "Franzoesisch // French Language" 
     | studienfach_tue == "Interkulturelle Deutsch-Franzoesische Studien // Intercultural German-French Studies" 
     | studienfach_tue == "Italienisch // Italian Language" | studienfach_tue == "Portugiesisch // Portuguese Language" 
     | studienfach_tue == "Spanisch // Spanish Language") & 
    (land == "France" | land == "Italy" | land == "Portugal" | land == "Spain" | land == "Argentina" | land == "Brazil" | land == "Chile"
     | land == "Colombia" | land == "Ecuador" | land == "Mexico" | land == "Peru" | land == "Uruguay")){
   score <- score + Land_Studiengang_Gewichtung
 }

if((studienfach_tue == "Lateinamerikastudien // Latin American Studies") &
   (land == "Argentina" | land == "Brazil" | land == "Chile"| land == "Colombia" | land == "Ecuador" 
  ...
  if(studienfach_tue == studienfach_in_2){ #falls das zweite Studienfach mit dem Fach des Tuebingers uebereinstimmt
    score <- score+Studienfach_Gewichtung #vergib Punktzahl fuer selbes Studienfach
  } else if(studienfach_fachbereich_tue == studienfach_fachbereich_in2 & !(studienfach_fachbereich_in == studienfach_fachbereich_tue)){
    ...
    score <- score+Fachbereich_Gewichtung #vergib Punkte fuer selben Fachbereich
  } else if(studienfach_fakultaet_tue == studienfach_fakultaet_in2 & !(studienfach_fakultaet_in == studienfach_fakultaet_tue)){
    ...
    score <- score+Fakultaet_Gewichtung #vergib Punkte fuer selbe Fakultaet
  } else if(berg_tal(studienfach_fakultaet_tue, studienfach_fakultaet_in2) & !(berg_tal(studienfach_fakultaet_tue, studienfach_fakultaet_in))){
  ...
  if(!(is.na(studienfach_tue_2))){  #hole Fachbereich und Fakultaet fuer den zweiten Studiengang mittels studiengang_funktion
  ...
  if(studienfach_tue_2 == studienfach_in){ #falls das zweite Studienfach mit dem Fach des Incomings uebereinstimmt
    score <- score+Studienfach_Gewichtung  #vergib Punktzahl fuer selbes Studienfach
  } else if(studienfach_fachbereich_tue2 == studienfach_fachbereich_in & !(studienfach_fachbereich_in == studienfach_fachbereich_tue)){
    ...
    score <- score+Fachbereich_Gewichtung #vergib Punkte fuer selben Fachbereich
  } else if(studienfach_fakultaet_tue2 == studienfach_fakultaet_in & !(studienfach_fakultaet_in == studienfach_fakultaet_tue)){
    ...
    score <- score+Fakultaet_Gewichtung #vergib Punkte fuer selbe Fakultaet
  }else if(berg_tal(studienfach_fakultaet_tue2, studienfach_fakultaet_in) & !(berg_tal(studienfach_fakultaet_tue, studienfach_fakultaet_in))){
  ...
  if(!(is.na(studienfach_in_2))){ #hole Fachbereich und Fakultaet fuer den zweiten Studiengang mittels studiengang_funktion
    studienfach_informationen_in2 =studiengang_funktion(studienfach_in_2) 
    studienfach_fachbereich_in2 = studienfach_informationen_in2[1]
    studienfach_fakultaet_in2 = studienfach_informationen_in2[2]
    
    if(studienfach_tue_2 == studienfach_in_2){ #falls die beiden zweiten Studienfaecher uebereinstimmen
      score <- score+Studienfach_Gewichtung #vergib Punktzahl fuer selbes Studienfach
    } else if(studienfach_fachbereich_tue2 == studienfach_fachbereich_in2 & !(studienfach_fachbereich_in2 == studienfach_fachbereich_tue)
      ...
      score <- score+Fachbereich_Gewichtung #vergib Punkte fuer selben Fachbereich
    } else if(studienfach_fakultaet_tue2 == studienfach_fakultaet_in2 & !(studienfach_fakultaet_in2 == studienfach_fakultaet_tue)
  ...
  if((studienfach_tue_2 == "Japanologie // Japanese Studies") & (land == "Japan")){
    score <- score + Land_Studiengang_Gewichtung
  }
  if((studienfach_tue_2 == "Koreanistik // Korean Studies") & (land == "South Korea")){
    score <- score + Land_Studiengang_Gewichtung
  }
  if((studienfach_tue_2 == "Indologie // Indology") & (land == "India")){
    score <- score + Land_Studiengang_Gewichtung
  }
  ...
  if((studienfach_tue_2 == "American Studies") & (land == "USA")){
    score <- score + Land_Studiengang_Gewichtung
  }
 ...
 # if((studienfach_tue_2 == "Anglistik // English Studies" | studienfach_tue_2 == "Englisch // English language" | studienfach_tue_2 == "English Linguistics") 
  ...
  if((studienfach_tue_2 == "Slavistik // Slavic Studies" | studienfach_tue_2 == "Russisch // Russian Language") & 
     (land == "Latvia" | land == "Lithuania" | land == "Estonia" | land == "Russia" | land == "Ukraine" | land == "Belarus" 
      | land == "Poland" | land == "Slovakia" | land == "Slovenia" | land == "Czech Republic" | land == "Croatia" | land == "Serbia"
      | land == "Bulgaria" | land == "Romania")){
    score <- score + Land_Studiengang_Gewichtung
  ...
  if((studienfach_tue_2 == "Romanistik // Romance Language and Literature" | studienfach_tue_2 == "Franzoesisch // French Language" 
      | studienfach_tue_2 == "Interkulturelle Deutsch-Franzoesische Studien // Intercultural German-French Studies" 
      | studienfach_tue_2 == "Italienisch // Italian Language" | studienfach_tue_2 == "Portugiesisch // Portuguese Language" 
  ...
  for(h in 1:3){ #gehe alle Hobbies des Incomings durch
  if(hobbies_in[[h]] == hobbies_tue[[1]] | hobbies_in[[h]] == hobbies_tue[[2]] | hobbies_in[[h]] == hobbies_tue[[3]]){
    score <- score+Hobby_Gewichtung #falls das Hobby mit einem Hobby des Tuebingers uebereinstimmt, vergib Punktzahl fuer gleiches Hobby
  } else
  if(hobbykategorie_funktion(hobbies_in[[h]],hobbies_tue,hobbies_in,h)){ #andernfalls ueberpruefe mit hobbykategorie_funktion, ob es  
                                                                        #in der selben Kategorie ist wie ein Hobby des Tuebingers
    score <- score+Hobbykategorie_Gewichtung #falls ja, vergib Punktzahl fuer selbe Hobbykategorie
    }
  }
  ...
  if(tuebingen[Spalte_Geschlecht_Tuebingen]==incoming[Spalte_Geschlecht_Incoming]){
    score <- score+Geschlecht_Gewichtung #falls selbes Geschlecht, vergib Punkte
  }
  ...
  if(!(is.na(tuebingen[Spalte_Uni_Tuebingen_Freitext]))) {tuebingen[Spalte_Uni_Tuebingen] <- tuebingen[Spalte_Uni_Tuebingen_Freitext]}
  if(!(is.na(incoming[Spalte_Uni_Incoming_Freitext]))) {incoming[Spalte_Uni_Incoming] <- incoming[Spalte_Uni_Incoming_Freitext]}
  
  if(!(is.na(tuebingen[Spalte_Uni_Tuebingen]))){ #falls der Tuebinger ein Auslandssemester gemacht hat
  if(tuebingen[Spalte_Uni_Tuebingen]==incoming[Spalte_Uni_Incoming]){
    score <- score+Austauschuni_Gewichtung #vergib Punkte falls Tuebinger an Uni von Incoming war
  }else
  if(tuebingen[Spalte_Land_Tuebingen]==incoming[Spalte_Land_Incoming]){
    score <- score+Austauschland_Gewichtung #andernfalls vergib Punkte falls Tuebinger im Herkunftsland des Incomings war
  }
  }
  ...
  if(is.na(incoming[[Spalte_Sprache_Incoming]])){
   ...
   sprachen_tuebingen <- strsplit(toString(tuebingen[[Spalte_Sprachen_Tuebingen]]),";")
   sprachen_incoming <- strsplit(toString(incoming[[Spalte_Sprache_Incoming]]),";")
   ...
   for(s in 1:anzahl_sprachen_incoming){
    ...
    if(sprachen_incoming[[1]][s] != "German" & sprachen_incoming[[1]][s] != "English"){
     if(sprachen_incoming[[1]][s] %in% sprachen_tuebingen[[1]]){
       score <- score + Sprache_Gewichtung #falls Sprache identisch und nicht Deutsch oder Englisch, vergib fuer jede gleiche Sprache Punkte
     }
    }
   }
  ...
  if(ankunftsdatum_unterschied <= 0){ #falls der Tuebinger vor dem Buddy da ist
    score <- score + Ankunftsdatum_0Wochen_Gewichtung
  }else
    if(ankunftsdatum_unterschied <= 14){ #falls der Buddy nur bis zu 2 Wochen alleine ist
      score <- score + Ankunftsdatum_2Wochen_Gewichtung
    }else
      if(ankunftsdatum_unterschied <= 21){#falls der Buddy zwischen 2 und 3 Wochen alleine ist
  ...
  if(is.na(incoming[Spalte_Studienabschluss_Incoming])){
    incoming[Spalte_Studienabschluss_Incoming] = "none"
     ...
     if(tuebingen[Spalte_Studienabschluss_Tuebingen] == "Doktorand/in // PhD Student?" & incoming[Spalte_Studienabschluss_Incoming] == "PhD Student?"){
     score <- score + StudienabschlussPhD_Gewichtung #falls beide PhD, so werden Punkte vergeben
   }
...
studiengang_funktion <- function(studiengang){
  ...
  if(is.na(studiengang)) {return(c(NA,NA))} else
  ...
  if(studiengang == "Evangelische Theologie // Protestant Theology" | studiengang == "Judaistik // Jewish Studies"){
  ...
  else if(studiengang == "Biomedical Technologies" | studiengang == "Medizin // Medicine"
     | studiengang == "Medizin - Biotechnologie // Medicine - Biotechnology" | studiengang == "Medizintechnik // Medical Technology"
     | studiengang == "Molekulare Medizin // Molecular Medicine" | studiengang == "Neuro- und Verhaltenswissenschaft // Neuro- and Behavioral Science"
     | studiengang == "Zahnmedizin // Dentistry" | studiengang == "Zellulaere und Molekulare Neurowissenschaften // Cellular and Molecular Neuroscience"
     | studiengang == "Experimentelle Medizin // Experimental Medicine (PhD)" | studiengang == "Hebammenwissenschaft // Midwifery"
     | studiengang == "Medizinische Strahlenwissenschaften // Medical Radiation Sciences"
     | studiengang == "Neuronale Informationsverarbeitung // Neural Information Processing" | studiengang == "Pflege // Nursing") {
    return(c("Medizin",F4))
  }
  ...
  else if(studiengang == "aegyptologie / Egyptology" | studiengang == "Archaeologie des Mittelalters // Archaeology of the Middle Ages"
     | studiengang == "IANES Vorderasiatische Archaeologie // Near Eastern Archaeology" | studiengang == "Klassische Archaeologie // Classical Archaeology"
     | studiengang == "Klassische Philologie // Classical Philology" | studiengang == "Kunstgeschichte // Art History"
     | studiengang == "Musikwissenschaft // Music" | studiengang == "Naturwissenschaftliche Archaeologie // Scientific Archaeology"
     | studiengang == "Religionswissenschaft // Religious Studies" | studiengang == "Ur- und Fruehgeschichte // Prehistory"
     | studiengang == "Ur- und Fruehgeschichtliche Archaeologie // Prehistorical Archaeology" | studiengang == "Vorderasiatische Archaeologie // Near Eastern Archaeology"
     | studiengang == "Altorientalische Philologie // Ancient Eastern Philology" | studiengang == "Griechisch // Greek"
     | studiengang == "Latein // Latin"){
    return(c("Altertums- und Kunstwissenschaften",F5))
  }
  
  else if(studiengang == "Ethnologie // Social and Cultural Anthropology" | studiengang == "Indologie // Indology" 
     | studiengang == "Japanologie // Japanese Studies" | studiengang == "Koreanistik // Korean Studies"
     ...
     | studiengang == "Sinologie / Chinese Studies"  | studiengang == "Islamwissenschaften // Islamic Studies"
  ...
  else if(studiengang == "Allgemeine Rhetorik // Rhetorics" | studiengang == "Medienwissenschaft // Media Science" |
     studiengang == "Philosophie // Philosophy"){
    return(c("Philosophie - Rhetorik - Medien",F5))
  }
  
  else if(studiengang == "American Studies" | studiengang == "Anglistik // English Studies" | studiengang == "Germanistik // German Studies"
     | studiengang == "Internationale Literaturen // International Literatures" | studiengang == "Romanistik // Romance Language and Literature"
     | studiengang == "Skandinavistik // Scandinavian Studies" | studiengang == "Slavistik // Slavic Studies" 
     | studiengang == "Cultures of the Global South" | studiengang == "Deutsch als Zweitsprache: Sprachdiagnostik und Sprachfoerderung // German as a Foreign Language: Linguistic Assessment and Language Support"
     | studiengang == "Deutsch // German Language" | studiengang == "Deutsche Literatur // German Literature" | studiengang == "Englisch // English Language"
     | studiengang == "English Linguistics" | studiengang == "Franzoesisch // French Language" | studiengang == "Interkulturelle Deutsch-Franzoesische Studien // Intercultural German-French Studies"
     | studiengang == "Italienisch // Italian Language" | studiengang == "Lateinamerikastudien // Latin American Studies"
     | studiengang == "Literatur- und Kulturtheorie // Literary and Cultural Theory" | studiengang == "Portugiesisch // Portuguese Language"
     | studiengang == "Russisch // Russian Language" | studiengang == "Spanisch // Spanish Language" ){
  ...
  else if(studiengang == "Computerlinguistik // Computational Linguistics" | studiengang == "Sprachwissenschaften // Linguistics"){
  ...
  #------------------------------------------------------------------
     ...
     | studiengang == "Empirische Kulturwissenschaft // Empirical Cultural Studies" | studiengang == "Erwachsenenbildung // Adult Education"
     | studiengang == "Erziehungswissenschaft // Educational Science" | studiengang == "Politikwissenschaft // Political Science"
     ...
     | studiengang == "Sozialpaedagogik // Educational Science" | studiengang == "Soziologie // Sociology"
     | studiengang == "Sportwissenschaft // Sports Science" | studiengang == "Comparative + Middle East Politics and Society (CMEPS)"
  ...
  else if (studiengang == "Accounting and Finance (M.Sc.)" | studiengang == "Business Administration" | studiengang == "Economics"
      | studiengang == "European Economics (M.Sc.)" | studiengang == "European Management (M.Sc.)" | studiengang == "General Management (M.Sc.)"
      | studiengang == "Management & Economics (M.Sc.)" | studiengang == "International Business (M.Sc.)"
      | studiengang == "International Business Administration (BA)" | studiengang == "International Economics (BA)"
      | studiengang == "International Economics (M.Sc.)" | studiengang == "Data Science in Business and Economics"
      | studiengang == "Economics and Business Administration" | studiengang == "Economics and Finance"){
  ...
  else if(studiengang == "Biochemie // Biochemistry" | studiengang == "Pharmaceutical Sciences and Technologies"
     | studiengang == "Pharmazie // Pharmacy"){
    return(c("Pharmazie und Biochemie",F7))
  }
  
  else if(studiengang == "Bioinformatik // Bioinformatics" | studiengang == "Informatik // Computer Science"
     | studiengang == "Kognitionswissenschaft // Cognition Science" | studiengang == "Medieninformatik // Media Informatics"
     | studiengang == "Machine Learning" | studiengang == "Medizininformatik // Medical Informatics"){
    return(c("Informatik",F7))
  }
  
  else if(studiengang == "Biologie // Biology" | studiengang == "Nano-Science" 
          | studiengang == "Evolution und oekologie // Evolution and Ecology" | studiengang == "Mikrobiologie // Microbiology"
  ...
  else if(studiengang == "Geographie // Geography" | studiengang == "Geologie // Geology" | studiengang == "Geooekologie // Geoecology"
     | studiengang == "Palaeoanthropologie // Palaeoanthropology" | studiengang == "Umweltwissenschaften // Environmental Science"
     | studiengang == "Humangeographie // Global Studies" | studiengang == "Applied + Environmental Geoscience AEG" 
  ...
  else if(studiengang == "Physik // Physics" | studiengang == "Mathematical Physics" | studiengang == "Naturwissenschaft und Technik // Natural Sciences and Technology"
     | studiengang == "Astro and Particle Physics" | studiengang == "Astronomie // Astronomy"){
    return(c("Physik",F7))
  }
  
  else if(studiengang == "Psychologie // Psychology" | studiengang == "Schulpsychologie // School Psychology"){
  ...
  else if(studiengang == "Islamische Theologie // Islamic Theology" | studiengang == "Islamische Religionslehre // Islamic Religious Education"){
...
berg_tal <- function(fakultaet1, fakultaet2){
  ...
  if((fakultaet1=="Medizinische Fakultaet" & fakultaet2=="Mathematisch-Naturwissenschaftliche Fakultaet")|
     (fakultaet2=="Medizinische Fakultaet" & fakultaet1=="Mathematisch-Naturwissenschaftliche Fakultaet")){
    return(TRUE)
  }else if(fakultaet1=="Medizinische Fakultaet" | fakultaet2=="Mathematisch-Naturwissenschaftliche Fakultaet"){#ueberpruefen ob einer Bergler ist
    return(FALSE)
  }else{#andernfalls muessen beide im Tal studieren
    return(TRUE)
  }
...
hobbykategorie_funktion <- function(hobby_in, hobbies_tue, hobbies_in, h)
{
  
  Hobbies=c("Creativity","Culture","Media","Music","Sports")
  for(hobby in Hobbies){
  if(startsWith(toString(hobby_in), hobby) & ((startsWith(toString(hobbies_tue[[1]]), hobby) & hobby_nicht_genutzt(h, hobbies_in, hobby))
                                    | (startsWith(toString(hobbies_tue[[2]]), hobby) & hobby_nicht_genutzt(h, hobbies_in, hobby))
                                    | (startsWith(toString(hobbies_tue[[3]]), hobby) &  hobby_nicht_genutzt(h, hobbies_in, hobby)))){
